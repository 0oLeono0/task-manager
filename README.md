...
- Проверить, что задачи загружаются из backend.
- Создать новую задачу и убедиться, что она появилась в списке.
- Переключить `completed` у задачи и убедиться, что состояние визуально изменилось.
- Удалить задачу и убедиться, что она исчезла из списка.
- Удалить все задачи и проверить empty state: `Задач пока нет`.
- Остановить backend и проверить ошибку загрузки задач.
- Проверить, что повторные клики по create/update/delete не отправляют дубли.
